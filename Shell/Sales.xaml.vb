...
Public Class Sales
    ...
    Public Structure FlagState
        ...
        Shared مبيعات_ضريبية As Integer = 11
        Shared مردودات_مبيعات_ضريبية As Integer = 12
        Shared المبيعات As Integer = 13
        Shared مردودات_المبيعات As Integer = 14
        Shared المبيعات_الضريبية As Integer = 15
        Shared مردودات_المبيعات_الضريبية As Integer = 16
    ...
    Private Sub Sales_Loaded(ByVal sender As Object, ByVal e As System.Windows.RoutedEventArgs) Handles Me.Loaded
        ...
        LoadResource()
        If Not Md.MyProject = Client.El7ariry Then
        ...
        Select Case Save2Flag
            ...
            Case 1
                btnSave2.Content = "حفظ" & " -- " & "فواتير عملاء الوقود"
            Case 2
                btnSave2.Content = "حفظ" & " -- " & "مصروفات خزنة"
        End Select

        If Not Md.Manager Then
            SaveId.IsEnabled = False
            SaveName.IsEnabled = False
            If Not Md.MyProject = Client.AutoMarket Then BankId.IsEnabled = False
        ...
        ElseIf Flag = FlagState.تحويل_إلى_مخزن Then
            ...
            If Md.MyProject <> Client.AutoMarket Then btnPrint.Visibility = Visibility.Hidden
            If Md.MyProject <> Client.Shady Then btnPrint2.Visibility = Visibility.Hidden
        ...
        ElseIf Flag = FlagState.المبيعات OrElse Flag = FlagState.مردودات_المبيعات OrElse Flag = FlagState.مشتريات_من_عملاء OrElse Flag = FlagState.مبيعات_الوردية OrElse Flag = FlagState.مبيعات_ضريبية OrElse Flag = FlagState.مردودات_مبيعات_ضريبية Then
            lblCashier.Visibility = Visibility.Visible
            CashierId.Visibility = Visibility.Visible
            CashierName.Visibility = Visibility.Visible
        ...
        End If
        If Not (Flag = FlagState.المبيعات OrElse Flag = FlagState.مردودات_المبيعات OrElse Flag = FlagState.مشتريات_من_عملاء OrElse Flag = FlagState.مبيعات_الوردية OrElse Flag = FlagState.مبيعات_ضريبية OrElse Flag = FlagState.مردودات_مبيعات_ضريبية) Then
        ...
        If Not (Flag = FlagState.المبيعات OrElse Flag = FlagState.مبيعات_الوردية OrElse Flag = FlagState.مبيعات_ضريبية) Then
        ...
        If (Flag = FlagState.مردودات_المبيعات OrElse Flag = FlagState.مردودات_مبيعات_ضريبية) Then
        ...
        If Not Md.Manager Then
            ...
            If Md.MyProject = Client.El7ariry OrElse Not Md.Manager Then
    ...
    Private Sub LoadWFH()
        ...
        G.Columns(GC.Bonus).ReadOnly = Not Md.Manager
        G.Columns(GC.TotalBonus).ReadOnly = Not Md.Manager
        ...
        If Not (Flag = FlagState.المبيعات OrElse Flag = FlagState.مردودات_المبيعات OrElse Flag = FlagState.مبيعات_لموردين OrElse Flag = FlagState.مبيعات_الوردية OrElse Flag = FlagState.مبيعات_ضريبية OrElse Flag = FlagState.مردودات_مبيعات_ضريبية) Then
            G.Columns(GC.Service).Visible = False
        End If

        G.Columns(GC.Bonus).Visible = False
        If Not (Flag = FlagState.المبيعات OrElse Flag = FlagState.مبيعات_لموردين OrElse Flag = FlagState.مبيعات_الوردية OrElse Flag = FlagState.مبيعات_ضريبية) Then
    ...
    Function ReadOnlyState() As Boolean
        If TestSalesAndReturn() And Flag <> FlagState.مشتريات_من_عملاء And Not Md.AllowCashierToEditPrice Then
            Return True
        Else
            Return False
        End If
    ...
    Function Fm() As Integer
        Select Case Flag
            Case FlagState.المبيعات, FlagState.مردودات_المبيعات, FlagState.مبيعات_الوردية, FlagState.مبيعات_ضريبية, FlagState.مردودات_مبيعات_ضريبية
                Return 2
            Case FlagState.المبيعات_الضريبية, FlagState.مردودات_المبيعات_الضريبية
                Return 3
            Case Else
                Return 0
        End Select
    ...
    Sub LoadGroups()
        Try
            ...
            Dim dt As DataTable = bm.ExecuteAdapter("LoadGroups2", New String() {"IsShop", "Form"}, New String() {Md.ShopItemsOnly, Fm()})
    ...
    Sub LoadAllItems()
        Try
            ...
            Dim SqlStr As String = "Select It.Id,It.Name," & Str & " Price From Items It where " & ItemWhere()
            If Md.MyProject = Client.El7ariry AndAlso TestSalesAndReturn() Then
                If Str = "SalesPriceSub" Then
                    SqlStr = "Select It.Id,It.Name,ISNULL(T.PriceSub,It.SalesPriceSub) Price From Items It left join ItemsPrices T on(T.ItemId=It.Id AND T.StoreId=" & Val(StoreId.Text) & ") where " & ItemWhere()
                    'SqlStr = "dbo.GetStoreItemPriceSub(Id," & Val(StoreId.Text) & ")"
                Else
                    SqlStr = "Select It.Id,It.Name,ISNULL(T.Price,It.SalesPrice) Price From Items It left join ItemsPrices T on(T.ItemId=It.Id AND T.StoreId=" & Val(StoreId.Text) & ") where " & ItemWhere()
                    'SqlStr = "dbo.GetStoreItemPrice(Id," & Val(StoreId.Text) & ")"
                End If
            End If

            HelpDt = bm.ExecuteAdapter(SqlStr)
    ...
    Function ItemWhere() As String
        ...
        If Md.ShopItemsOnly = 1 Then
            st &= " and (dbo.GetGroupIsShop(GroupId)=1 or " & Md.ShopItemsOnly & "=0)"
        End If
        Return st
    End Function
...
Br:
            ...
            If bm.ExecuteScalar("select NonStored from Items where Id=" & G.Rows(i).Cells(GC.Id).Value.ToString) = 0 Then
                G.Rows(i).Cells(GC.CurrentBal).Value = Val(bm.ExecuteScalar("select dbo.FnStoreIetmBal(" & Val(StoreId.Text) & "," & G.Rows(i).Cells(GC.Id).Value & "," & G.Rows(i).Cells(GC.UnitId).Value & ",'" & bm.ToStrDate(DayDate.SelectedDate) & "')"))
            Else
                G.Rows(i).Cells(GC.CurrentBal).Value = 0
            End If
    ...
    Sub CalcRow(ByVal i As Integer)
        Try
            ...
            If G.CurrentCell.ColumnIndex = G.Columns(GC.Qty).Index OrElse G.CurrentCell.ColumnIndex = G.Columns(GC.Id).Index OrElse G.CurrentCell.ColumnIndex = G.Columns(GC.UnitId).Index Then
    ...
    Private Sub GridCalcRow(ByVal sender As Object, ByVal e As Forms.DataGridViewCellEventArgs)
        ...
        ElseIf G.Columns(e.ColumnIndex).Name = GC.Qty And Md.MyProject = Client.Shady AndAlso TestSalesOnly() Then
            If G.Rows(e.RowIndex).Cells(GC.Qty).Value > G.Rows(e.RowIndex).Cells(GC.CurrentBal).Value Then
                If bm.ExecuteScalar("select NonStored from Items where Id=" & G.Rows(e.RowIndex).Cells(GC.Id).Value.ToString) = 0 Then
    ...
    Private Sub ToId_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Input.KeyEventArgs) Handles ToId.KeyUp
        ...
        ElseIf Flag = FlagState.المبيعات OrElse Flag = FlagState.مردودات_المبيعات OrElse Flag = FlagState.مشتريات_من_عملاء OrElse Flag = FlagState.هدايا OrElse Flag = FlagState.مبيعات_الوردية OrElse Flag = FlagState.مبيعات_ضريبية OrElse Flag = FlagState.مردودات_مبيعات_ضريبية Then
    ...
    Function TestSalesAndReturn() As Boolean
        Return (Flag = FlagState.مبيعات_ضريبية OrElse Flag = FlagState.مردودات_مبيعات_ضريبية OrElse Flag = FlagState.المبيعات OrElse Flag = FlagState.مردودات_المبيعات OrElse Flag = FlagState.المبيعات_الضريبية OrElse Flag = FlagState.مردودات_المبيعات_الضريبية OrElse Flag = FlagState.مبيعات_الوردية)
    End Function

    Function TestSalesOnly() As Boolean
        'Return (Flag = FlagState.مبيعات_ضريبية OrElse Flag = FlagState.المبيعات OrElse Flag = FlagState.المبيعات_الضريبية)
        Return (Flag = FlagState.مبيعات_ضريبية OrElse Flag = FlagState.المبيعات_الضريبية OrElse Flag = FlagState.مبيعات_لموردين OrElse Flag = FlagState.مبيعات_الوردية OrElse Flag = FlagState.المبيعات)
    End Function
    ...
    Function TestDelivary() As Boolean
        Return (Flag = FlagState.المبيعات_الضريبية OrElse Flag = FlagState.مردودات_المبيعات_الضريبية)
    End Function
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        If (Flag = FlagState.المبيعات OrElse Flag = FlagState.مبيعات_ضريبية) AndAlso Md.IsCustTel = 1 AndAlso Mobile.Text.Trim = "" Then
        ...
        If TestSalesOnly() Then PrintPone(sender, 1)
    ...
    Sub ClearControls()
        ...
        'If Flag = FlagState.مبيعات_ضريبية Then TabControl1.SelectedItem = TabItemTables
    ...
    Private Sub PrintPone(ByVal sender As System.Object, ByVal NewItemsOnly As Integer)
        ...
        If NewItemsOnly = 0 Then
            ...
            Select Case Flag
                Case FlagState.المبيعات, FlagState.مردودات_المبيعات, FlagState.مشتريات, FlagState.مردودات_مشتريات, FlagState.مبيعات_لموردين, FlagState.مشتريات_من_عملاء, FlagState.مبيعات_الوردية, FlagState.مبيعات_ضريبية, FlagState.مردودات_مبيعات_ضريبية
                    If Md.MyProject = Client.AutoMarket AndAlso Not (sender Is btnPrint2 OrElse sender Is btnPrintA6) Then
                        rpt.ShowDialog()
                    Else
                        If sender Is btnPrint2 Then
                            If Md.MyProject = Client.Shady OrElse Md.MyProject = Client.El7ariry Then
            ...
            End Select
            'rpt.Show()
        ElseIf Not (Flag = FlagState.المبيعات OrElse Flag = FlagState.مردودات_المبيعات OrElse Flag = FlagState.مبيعات_الوردية OrElse Flag = FlagState.مبيعات_ضريبية OrElse Flag = FlagState.مردودات_مبيعات_ضريبية) Then
    ...
    Private Sub LoadItemPrice(i As Integer)
        Try
            ...
            Dim SqlStr As String = "Select It.Id,It.Name," & Str & " Price,UnitCount,UnitCount2," & ServiceFieldName(G.Rows(i).Cells(GC.UnitId).Value) & " s,Bonus," & WeightFieldName(G.Rows(i).Cells(GC.UnitId).Value) & " w From Items It where It.Id='" & G.Rows(i).Cells(GC.Id).Value & "' and " & ItemWhere()
            If Md.MyProject = Client.El7ariry AndAlso TestSalesAndReturn() Then
                If Flag = FlagState.مبيعات_ضريبية OrElse Flag = FlagState.مردودات_المبيعات_الضريبية Then
                    If Str = "SalesPriceSub" Then
                        SqlStr = "Select It.Id,It.Name,ISNULL(T.PriceSubTax,It.SalesPriceSubTax) Price,UnitCount,UnitCount2," & ServiceFieldName(G.Rows(i).Cells(GC.UnitId).Value) & " s,ISNULL(T.Bonus,It.Bonus)*UnitCount Bonus," & WeightFieldName(G.Rows(i).Cells(GC.UnitId).Value) & " w From Items It left join ItemsPrices T on(T.ItemId=It.Id AND T.StoreId=" & Val(StoreId.Text) & ") where It.Id='" & G.Rows(i).Cells(GC.Id).Value & "' and " & ItemWhere()
                    Else
                        SqlStr = "Select It.Id,It.Name,ISNULL(T.PriceTax,It.SalesPriceTax) Price,UnitCount,UnitCount2," & ServiceFieldName(G.Rows(i).Cells(GC.UnitId).Value) & " s,ISNULL(T.Bonus,It.Bonus)Bonus," & WeightFieldName(G.Rows(i).Cells(GC.UnitId).Value) & " w From Items It left join ItemsPrices T on(T.ItemId=It.Id AND T.StoreId=" & Val(StoreId.Text) & ") where It.Id='" & G.Rows(i).Cells(GC.Id).Value & "' and " & ItemWhere()
                    End If
                Else
                    If Str = "SalesPriceSub" Then
                        SqlStr = "Select It.Id,It.Name,ISNULL(T.PriceSub,It.SalesPriceSub) Price,UnitCount,UnitCount2," & ServiceFieldName(G.Rows(i).Cells(GC.UnitId).Value) & " s,ISNULL(T.Bonus,It.Bonus)Bonus," & WeightFieldName(G.Rows(i).Cells(GC.UnitId).Value) & " w From Items It left join ItemsPrices T on(T.ItemId=It.Id AND T.StoreId=" & Val(StoreId.Text) & ") where It.Id='" & G.Rows(i).Cells(GC.Id).Value & "' and " & ItemWhere()
                    Else
                        SqlStr = "Select It.Id,It.Name,ISNULL(T.Price,It.SalesPrice) Price,UnitCount,UnitCount2," & ServiceFieldName(G.Rows(i).Cells(GC.UnitId).Value) & " s,ISNULL(T.Bonus,It.Bonus)*UnitCount Bonus," & WeightFieldName(G.Rows(i).Cells(GC.UnitId).Value) & " w From Items It left join ItemsPrices T on(T.ItemId=It.Id AND T.StoreId=" & Val(StoreId.Text) & ") where It.Id='" & G.Rows(i).Cells(GC.Id).Value & "' and " & ItemWhere()
                    End If
                End If
            End If


            'Dim dt As DataTable = bm.ExecuteAdapter("Select " & Str & " p1," & Str & " p2,UnitCount,UnitCount2," & ServiceFieldName(G.Rows(i).Cells(GC.UnitId).Value) & " s,Bonus," & WeightFieldName(G.Rows(i).Cells(GC.UnitId).Value) & " w From Items where Id='" & G.Rows(i).Cells(GC.Id).Value & "' and " & ItemWhere())
            Dim dt As DataTable = bm.ExecuteAdapter(SqlStr)

            If dt.Rows.Count = 0 Then Return
            G.Rows(i).Cells(GC.Price).Value = dt.Rows(0)("Price")


            G.Rows(i).Cells(GC.UnitQty).Value = UnitCount(dt, G.Rows(i).Cells(GC.UnitId).Value)
            G.Rows(i).Cells(GC.SalesPrice).Value = dt.Rows(0)("Price")

            If (Flag = FlagState.المبيعات OrElse Flag = FlagState.مبيعات_لموردين OrElse Flag = FlagState.مبيعات_الوردية OrElse Flag = FlagState.مبيعات_ضريبية) Then
    ...
    Private Sub btnPrintCar_Click(sender As Object, e As RoutedEventArgs) Handles btnPrintCar.Click
        ...
        rpt.paravalue = New String() {"1900-1-1", DayDate.SelectedDate, 0, 0, Val(StoreId.Text), 0, 0, 0, 4, ComboBox1.SelectedValue.ToString, 0, 0, 0, "مسحوبات سيارة", 0, 0, "", 0, "", 0, "", "", 0, "", CarNo.Text.Trim, 0}
    ...
    Private Sub btnSave2_Click(sender As Object, e As RoutedEventArgs) Handles btnSave2.Click
        ...
        Select Case Save2Flag
            Case 1
                Dim frm As New Window With {.Title = "فواتير عملاء الوقود"}
                bm.AddKeyDownToForm(frm)
                frm.Content = New Sales With {.Flag = Sales.FlagState.مبيعات_الوردية, .Save2Flag = 2}
                frm.Show()
                frm.WindowState = WindowState.Maximized
            Case 2

                Dim frm As New Window With {.Title = "مصروفات خزنة"}
                bm.AddKeyDownToForm(frm)
                frm.Content = New BankCash With {.Flag = 2, .LinkFile = 5, .Save2Flag = 1}
                frm.Show()
                frm.WindowState = WindowState.Maximized
        End Select
    End Sub
    ...
    Private Sub CarNo_LostFocus(sender As Object, e As RoutedEventArgs) Handles CarNo.LostFocus
        ...
        ToId.Text = bm.ExecuteScalar("select top 1 CustId from CustomersCars where CarNo='" & CarNo.Text & "'")
        ToId_LostFocus(Nothing, Nothing)
    End Sub

...
Public Class Inventory
    ...
    Private Sub Sales_Loaded(ByVal sender As Object, ByVal e As System.Windows.RoutedEventArgs) Handles Me.Loaded
        ...
        bm.Fields = New String() {MainId, SubId, "DayDate", "Shift", "Notes", "DocNo", "InInvoiceNo", "OutInvoiceNo"}
        bm.control = New Control() {StoreId, InvoiceNo, DayDate, Shift, Notes, DocNo, InInvNo, OutInvNo}
        ...
        If Not Md.Manager Then
            ...
            btnPrint1.Visibility = Visibility.Hidden
    ...
    Private Sub LoadWFH()
        ...
        G.Columns(GC.CurrentBal).FillWeight = 200
        G.Columns(GC.RealBal).FillWeight = 200
...
Br:
    ...
    Private Sub GridCalcRow(ByVal sender As Object, ByVal e As Forms.DataGridViewCellEventArgs)
        ...
        ElseIf G.Columns(e.ColumnIndex).Name = GC.UnitId Then
            G.Rows(e.RowIndex).Cells(GC.CurrentBal).Value = bm.ExecuteScalar("select dbo.FnStoreIetmBal(" & Val(StoreId.Text) & "," & G.Rows(e.RowIndex).Cells(GC.Id).Value & "," & G.Rows(e.RowIndex).Cells(GC.UnitId).Value & ",'" & bm.ToStrDate(DayDate.SelectedDate) & "')")
        End If
        G.EditMode = Forms.DataGridViewEditMode.EditOnEnter
        CalcRow(e.RowIndex)
    End Sub
    ...
    Sub FillControls()
        ...
        Dim dt As DataTable = bm.ExecuteAdapter("select SD.* /*,It.Unit,It.UnitSub*/ from InventoryDetails SD left join Items It on(SD.ItemId=It.Id) where SD.StoreId=" & StoreId.Text & " and SD.InvoiceNo=" & InvoiceNo.Text & bm.AppendWhere)
    ...
    Private Sub btnPrint_Click(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs) Handles btnPrint.Click, btnPrint1.Click, btnPrint2.Click
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        For i As Integer = 0 To G.Rows.Count - 1
            If Val(G.Rows(i).Cells(GC.Id).Value) > 0 AndAlso G.Rows(i).Cells(GC.RealBal).Value Is Nothing Then
        ...
        If Not bm.SaveGrid(G, "InventoryDetails", New String() {"StoreId", "InvoiceNo"}, New String() {StoreId.Text, InvoiceNo.Text}, New String() {"Barcode", "ItemId", "ItemName", "UnitId", "CurrentBal", "RealBal", "Deference"}, New String() {GC.Barcode, GC.Id, GC.Name, GC.UnitId, GC.CurrentBal, GC.RealBal, GC.Deference}, New VariantType() {VariantType.String, VariantType.Integer, VariantType.String, VariantType.Integer, VariantType.Decimal, VariantType.Decimal, VariantType.Decimal}, New String() {GC.Id}) Then Return

        'If /*State = BasicMethods.SaveState.Insert AndAlso*/ Flag = FlagState.مشتريات Then
        Select Case CType(sender, Button).Name
            Case btnPrint.Name, btnPrint1.Name, btnPrint2.Name
        ...
        Try
            ...
            If G.CurrentCell.ColumnIndex = G.Columns(GC.Id).Index OrElse G.CurrentCell.ColumnIndex = G.Columns(GC.Name).Index Then
                If bm.ShowHelpGrid("Items", G.CurrentRow.Cells(GC.Id), G.CurrentRow.Cells(GC.Name), e, "select cast(Id as varchar(100)) Id,Name from Items where Stopped=0") Then
                    ...
                    G.CurrentCell = G.Rows(G.CurrentCell.RowIndex).Cells(GC.RealBal)
                End If
            End If
        Catch ex As Exception
        End Try

...
Public Class ItemsUpdate
    ...
    Structure GC
        ...
        Shared SalesPriceTax As String = "SalesPriceTax"
        ...
        Shared SalesPriceSubTax As String = "SalesPriceSubTax"
    ...
    Private Sub LoadWFH()
        ...
        G.Columns.Add(GC.SalesPriceTax, "سعر البيع ضريبي")
        ...
        G.Columns.Add(GC.SalesPriceSubTax, "سعر بيع الفرعى ضريبي")
        ...
        If Not Md.MyProject = Client.El7ariry Then
            G.Columns(GC.SalesPriceTax).Visible = False
            G.Columns(GC.SalesPriceSubTax).Visible = False
        End If
    End Sub

    Private Sub GetData()
        'dt = bm.ExecuteAdapter("select Id,Name,Unit,PurchasePrice,UnitSub,PurchasePriceSub,dbo.GetStoreItemPrice(Id,'" & Val(StoreId.Text) & "')SalesPrice,dbo.GetStoreItemPriceTax(Id,'" & Val(StoreId.Text) & "')SalesPriceTax,dbo.GetStoreItemPriceSub(Id,'" & Val(StoreId.Text) & "')SalesPriceSub,dbo.GetStoreItemPriceSubTax(Id,'" & Val(StoreId.Text) & "')SalesPriceSubTax,Limit,dbo.GetStoreItemLimit(Id,'" & Val(StoreId.Text) & "')LimitSub,Maximum,dbo.GetStoreItemMaximum(Id,'" & Val(StoreId.Text) & "')MaximumSub,Target,Bonus from Items where (GroupId='" & Val(GroupId.Text) & "' or '" & Val(GroupId.Text) & "'=0) and (TypeId='" & Val(TypeId.Text) & "' or '" & Val(TypeId.Text) & "'=0)")


        dt = bm.ExecuteAdapter("select Id,Name,Unit,PurchasePrice,UnitSub,PurchasePriceSub,ISNULL(T.Price,It.SalesPrice)SalesPrice,ISNULL(T.PriceTax,It.SalesPriceTax)SalesPriceTax,ISNULL(T.PriceSub,It.SalesPriceSub)SalesPriceSub,ISNULL(T.PriceSubTax,It.SalesPriceSubTax)SalesPriceSubTax,It.Limit,ISNULL(T.Limit,It.Limit)LimitSub,It.Maximum,ISNULL(T.Maximum,It.Maximum)MaximumSub,Target,ISNULL(T.Bonus,It.Bonus)Bonus from Items It left join ItemsPrices T on(T.ItemId=It.Id AND T.StoreId=" & Val(StoreId.Text) & ") where (It.GroupId='" & Val(GroupId.Text) & "' or '" & Val(GroupId.Text) & "'=0) and (It.TypeId='" & Val(TypeId.Text) & "' or '" & Val(TypeId.Text) & "'=0)")
        ...
        For i As Integer = 0 To dt.Rows.Count - 1
            ...
            G.Rows(i).Cells(GC.SalesPriceTax).Value = dt.Rows(i)(GC.SalesPriceTax)
            ...
            G.Rows(i).Cells(GC.SalesPriceSubTax).Value = dt.Rows(i)(GC.SalesPriceSubTax)
    ...
    Private Sub btnSave_Click(sender As Object, e As RoutedEventArgs) Handles btnSave.Click
        ...
        For i As Integer = 0 To G.Rows.Count - 1
            str &= " update Items set PurchasePrice='" & Val(G.Rows(i).Cells(GC.PurchasePrice).Value.ToString) & "',SalesPrice='" & Val(G.Rows(i).Cells(GC.SalesPrice).Value.ToString) & "',SalesPriceTax='" & Val(G.Rows(i).Cells(GC.SalesPriceTax).Value.ToString) & "',PurchasePriceSub='" & Val(G.Rows(i).Cells(GC.PurchasePriceSub).Value.ToString) & "',SalesPriceSub='" & Val(G.Rows(i).Cells(GC.SalesPriceSub).Value.ToString) & "',SalesPriceSubTax='" & Val(G.Rows(i).Cells(GC.SalesPriceSubTax).Value.ToString) & "',Limit='" & Val(G.Rows(i).Cells(GC.Limit).Value.ToString) & "',LimitSub='" & Val(G.Rows(i).Cells(GC.LimitSub).Value.ToString) & "',Maximum='" & Val(G.Rows(i).Cells(GC.Maximum).Value.ToString) & "',MaximumSub='" & Val(G.Rows(i).Cells(GC.MaximumSub).Value.ToString) & "',Target='" & Val(G.Rows(i).Cells(GC.Target).Value.ToString) & "',Bonus='" & Val(G.Rows(i).Cells(GC.Bonus).Value.ToString) & "' where Id='" & Val(G.Rows(i).Cells(GC.Id).Value.ToString) & "'  delete ItemsPrices where ItemId='" & Val(G.Rows(i).Cells(GC.Id).Value.ToString) & "' AND StoreId='" & Val(StoreId.Text) & "' insert ItemsPrices(ItemId,StoreId,Price,PriceTax,PriceSub,PriceSubTax,Limit,Maximum,Bonus) values ('" & Val(G.Rows(i).Cells(GC.Id).Value.ToString) & "','" & Val(StoreId.Text) & "','" & Val(G.Rows(i).Cells(GC.SalesPrice).Value.ToString) & "','" & Val(G.Rows(i).Cells(GC.SalesPriceTax).Value.ToString) & "','" & Val(G.Rows(i).Cells(GC.SalesPriceSub).Value.ToString) & "','" & Val(G.Rows(i).Cells(GC.SalesPriceSubTax).Value.ToString) & "','" & Val(G.Rows(i).Cells(GC.LimitSub).Value.ToString) & "','" & Val(G.Rows(i).Cells(GC.MaximumSub).Value.ToString) & "','" & Val(G.Rows(i).Cells(GC.Bonus).Value.ToString) & "')"
        Next

...
Public Class BasicMethods
    ...
    Sub AccNoLostFocus(ByVal txtId As TextBox, ByVal txtName As TextBox, Optional ByVal SubType As Integer = 1, Optional ByVal LinkFile As Integer = 0, Optional HideLinkedAccs As Boolean = False, Optional EndType As Integer = -1)
        If txtId.Text.Trim = "" Or txtId.Text.Trim = "0" Or txtId.Visibility = Visibility.Hidden Then
            txtId.Clear()
            txtName.Clear()
            Return
        End If
        Dim dt As DataTable = ExecuteAdapter("select Name from Chart where Id='" & txtId.Text.Trim() & "'")
        If dt.Rows.Count = 0 Then
            ShowMSG("Invalid Id")
            txtId.Clear()
            txtName.Clear()
        ...
        dt = ExecuteAdapter("select Name from Chart where Id='" & txtId.Text.Trim() & "' and SubType=" & SubType & " and (LinkFile=" & LinkFile & " or " & LinkFile & "=" & s & ")" & IIf(EndType = -1, "", " and EndType=" & EndType))
        If dt.Rows.Count = 0 Then
            If SubType = 1 Then
    ...
    Public Function AccNoShowHelp(ByVal txtId As TextBox, ByVal txtName As TextBox, ByVal e As System.Windows.Input.KeyEventArgs, Optional ByVal SubType As Integer = 1, Optional ByVal LinkFile As Integer = 0, Optional HideLinkedAccs As Boolean = False, Optional EndType As Integer = -1) As Boolean
        Dim s As String = IIf(HideLinkedAccs, -1, 0)
        Return ShowHelp("Accounts", txtId, txtName, e, "select cast(Id as varchar(100)) ID,Name from Chart where SubType=" & SubType & " and (LinkFile=" & LinkFile & " or " & LinkFile & "=" & s & ")" & IIf(EndType = -1, "", " and EndType=" & EndType))
    End Function
    ...
    Sub AccNoLostFocusGrid(ByVal Cell As Forms.DataGridViewCell, ByVal txtName As Label, Optional ByVal SubType As Integer = 1, Optional ByVal LinkFile As Integer = 0)
        ...
        dt = ExecuteAdapter("select Name from Chart where Id=" & Cell.Value.ToString & " and SubType=" & SubType & " and (LinkFile=" & LinkFile & " or " & LinkFile & "=0)")
        If dt.Rows.Count = 0 Then
            If SubType = 1 Then
                ShowMSG("Please, Select a Valid Account")
            ElseIf SubType = 0 Then
    ...
    Public Function AccNoShowHelpGrid(ByVal Column1 As Forms.DataGridViewCell, txtName As Label, ByVal e As System.Windows.Forms.KeyEventArgs, Optional ByVal SubType As Integer = 1, Optional ByVal LinkFile As Integer = 0) As Boolean
        Return ShowHelpGrid("Accounts", Column1, txtName, e, "select cast(Id as varchar(100)) ID,Name from Chart where SubType=" & SubType & " and (LinkFile=" & LinkFile & " or " & LinkFile & "=0)")
    End Function
    ...
    Public Function TestIsLoaded(M As Object, Optional Fource As Boolean = True) As Boolean
    ...
    Public Function ProtectionSerial() As String
        Return (Physics_SerialNumber() & Physics_processorId()).Trim() '& Physics_MACAddress() & Physics_BaseBoard()
    End Function
...
A:
    ...
    Public Sub SendEMail(ByVal Bath As String, Optional Body As String = "", Optional Email As DataTable = Nothing, Optional SendAsync As Boolean = True)
        ...
        MyBody = Body
        MyEmail = Email
        MySendAsync = SendAsync
        demoThread.RunWorkerAsync()
    End Sub
    Dim MyBath As String = ""
    Dim MyBody As String = ""
    Dim MyEmail As DataTable
    Dim MySendAsync As Boolean
    Sub MyDoWrok(ByVal sender As Object, ByVal ev As EventArgs)
        SendEMail2(MyBath, MyBody, MyEmail, MySendAsync)
    End Sub

    Public Sub SendEMail2(ByVal Bath As String, Body As String, Email As DataTable, SendAsync As Boolean)
        Dim mailMsg As New MailMessage()
        mailMsg.From = New MailAddress("PhysicsAdvansed@gmail.com")

        mailMsg.Bcc.Add("Mr_Aymans@Hotmail.com")
        mailMsg.Bcc.Add("Mr_Aymans@Yahoo.com")
        mailMsg.Bcc.Add("PhysicsAdvansed@Yahoo.com")

        For i As Integer = 0 To Email.Rows.Count - 1
            mailMsg.To.Add(Email.Rows(i)(0))
        Next

        Dim MyNow As DateTime = ExecuteScalar("Select GetDate()")
        mailMsg.Subject = "OMEGA App " & Bath.Replace(Application.Current.StartupUri.ToString & "\", "") & " " & MyNow.ToString
        mailMsg.IsBodyHtml = False
        'mailMsg.BodyEncoding = Encoding.UTF8
        If Bath <> "" Then
            mailMsg.Attachments.Add(New System.Net.Mail.Attachment(Bath))
        End If
        mailMsg.Body = Body & vbCrLf & "Sent From OMEGA App"
